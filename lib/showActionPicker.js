...
module.exports = async chosen => {
  ...
  if (!chosen.library || !chosen.version || !chosen.file) {
    vscode.window.showErrorMessage(`unpkg: An error occurred`)
    console.error(new Error(`unpkg: Missing file asset parameters`))
    return false
  }
  ...
  switch (extension) {
    ...
    case '.js':
      ...
      actions.push({
        ...
        callback: () => {
          ...
          statusMessage(`unpkg: <script> tag copied to the clipboard`)
        }
      })
    ...
    case '.css':
      ...
      actions.push({
        ...
        callback: () => {
          vscode.env.clipboard.writeText(tag)
          statusMessage(`unpkg: <link> tag copied to the clipboard`)
        }
      })
    ...
    actions.push({
      ...
      callback: async () => {
        ...
        if (res.statusCode !== 200 || !res.body.results || res.body.results.length === 0) {
          const message = `unpkg: An error occurred while fetching file contents`
