...
module.exports = async libraryName => {
  ...
  // Configuration
  const config = vscode.workspace.getConfiguration('unpkg')
  ...
  return vscode.window.withProgress({
    title: `unpkg: Fetching library ${libraryName}`,
    location: vscode.ProgressLocation.Notification,
    cancellable: true
  }, async (progress, token) => {
    token.onCancellationRequested(() => {
      console.debug('unpkg: Fetch was cancelled by user')
    ...
    // res.body.versions
    res.body.assets = Object.entries(res.body.versions).map(el => ({ version: el[1].version, files: [el[1].main, el[1].style] })).sort((b, a) => a.version.replace(/\d+/g, n => +n + 100000)
      .localeCompare(b.version.replace(/\d+/g, n => +n + 100000)));
    delete res.body.versions;
    // Reject error if bad request
    if (res.statusCode !== 200) {
      const message = `unpkg: An error occurred`
    ...
    if (body.length === 0) {
      vscode.window.showErrorMessage(`unpkg: ${libraryName} was not found`)
      return false
    }
